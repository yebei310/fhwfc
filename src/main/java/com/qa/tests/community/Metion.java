...
//搜索联想接口
public class Metion extends TestBase {
    //設置请求超时时间
    ...
    public void filterCon(String contenx,String loginUrl,String username,String password)throws Exception{
        ...
        HttpPost post = new HttpPost("https://www.izhiliao.com/community/api/loupan/metion?siteId=3066&keyword=国风美仑");
            ...
            if (res.getStatusLine().getStatusCode() == HttpStatus.SC_OK){
               ...
               JSONObject jsonObject =JSONObject.parseObject(result);
                ...
                if (msgValue.equals("操作成功")){
                    JSONArray jsonArrayKerworlds = jsonObject.getJSONArray("data");
                    Reporter.log("返回jsonArrayKerworlds:"+jsonArrayKerworlds);

                    ArrayList<String> keyworlds = new ArrayList<String>();
                    for (int i =0;i<jsonArrayKerworlds.size();i++){
                       String keyworld =  jsonArrayKerworlds.getJSONObject(i).getString("areaName");
                        keyworlds.add(keyworld);
                        System.out.println("联想词：================="+keyworld);
                    }
                    String expectWorld="马驹桥";
                    if (keyworlds.contains(expectWorld)){
                        System.out.println("找到"+expectWorld);
                        Reporter.log("找到"+expectWorld);
                        Assert.assertTrue(true);

                    }else {
                        System.out.println("没有找到"+expectWorld);
                        Reporter.log("没有找到"+expectWorld);
