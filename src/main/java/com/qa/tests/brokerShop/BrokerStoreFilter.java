...
// 店铺经纪人: wap端获取更多筛选条件
public class BrokerStoreFilter extends TestBase {
    //设置请求超时时间
...
//        res = restClient.postApidetail(host+loginUrl,userJsonString,postHeader);
        ...
        HttpPost post = new HttpPost("https://bj.izhiliao.com/broker/store/filter?type=1");
        ...
        if (res.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
            ...
            JSONArray jsonArray = jsonData.getJSONArray("fitment");
            String decoration = jsonArray.getJSONObject(1).getString("精装");
            String expectDecoration = "精装";
            if (decoration.contains(expectDecoration)){
                Reporter.log("获取到装修条件"+expectDecoration);
                System.out.println("获取到装修条件"+expectDecoration);
                Assert.assertTrue(true);
            }else {
                Reporter.log("获取装修条件失败");
                System.out.println("获取装修条件失败");
